...
@Component({
  selector: 'app-quickcontacts',
  templateUrl: './quickcontacts.component.html',
  styleUrls: ['./quickcontacts.component.css']
})
export class QuickcontactsComponent implements OnInit {
  ...
  public delete(i){
    this.FamilyList.quickReferences.splice(i,1)
    this.http.put(this.GL.HOST+'userdetail', this.FamilyList,{ observe: 'response' }).subscribe((data)=>{
      this.getAllFamilyMember();
      this.GL.addmessage('details deleted Successfully.','S');
      this.row = 1;
    })
  }

  public add(){
    this.Family = new QRef();
    //this.FamilyList.quickReferences.push( this.Family);
  }
  public edit(L){
    this.row = 0;
    this.Family = L;
